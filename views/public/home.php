<!DOCTYPE html>
<html>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
<link rel="stylesheet" href="./assets/css/templateAdmin.css">
<link rel="stylesheet" href="./assets/css/templatePublic.css">
<link rel="stylesheet" href="assets/css/home.css">
<link rel="icon" type="image/png" sizes="18x18" href="./assets/pictures/logo2.png">
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-eOJMYsd53ii+scO/bJGFsiCZc+5NDVN2yr8+0RDqr0Ql0h+rP48ckxlpbzKgwra6" crossorigin="anonymous">

</head>
<body>
    <div class="lignes">
        <div class="l1"></div>
        <div class="l2"></div>
    </div>

    <div class="container-first">
        <h1><span>Un </span><span>Chef </span><span>à </span><span>la </span><span>maison</span></h1>
        <div class="container-btns">
        <a href="" ></a>
            <a href="#" type="submit" name="soumis" class="btn-first b1">Vos Chefs</a>
            <a href="index.php?action=accueil" type="submit" name="soumis" class="btn-first b2">Leurs menus</a>
        </div>
    </div>

    <img src="./assets/pictures/logo2.png" alt="" class="logo">


    <ul class="medias">
        <li class="bulle"><img src="./assets/pictures/fb2.png" alt="Facebook" class="logo-medias"></li>
        <li class="bulle"><img src="./assets/pictures/twitter2.png" alt="Twitter" class="logo-medias"></li>
        <li class="bulle"><img src="./assets/pictures/insta2.png" alt="Instagram" class="logo-medias"></li>
        <li class="bulle"><img src="./assets/pictures/github.png" alt="Github" class="logo-medias"></li>
    </ul>


<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.5.1/jquery.min.js" integrity="sha512-bLT0Qm9VnAYZDflyKcBaQ2gg0hSYNQrJ8RilYldYQ1FxQYoCLtUjuuRuZo+fjqhx/qtq/1itJ0C2ejDxltZVFg==" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.9.1/dist/umd/popper.min.js" integrity="sha384-SR1sx49pcuLnqZUnnPwx6FCym0wLsk5JZuNx2bPPENzswTNFaQU1RDvt3wT4gWFG" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta3/dist/js/bootstrap.min.js" integrity="sha384-j0CNLUeiqtyaRmlzUHCPZ+Gy5fQu0dQ6eZ/xAww941Ai1SxSY+0EQqNXNE6DZiVc" crossorigin="anonymous"></script>
<script src="./assets/js/templateAdmin.js"></script>
<!-- <script src="./assets/js/templatePublic.js"></script> -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.6.1/gsap.min.js"></script>
<script src="./assets/js/home.js"></script>
<script src="https://kit.fontawesome.com/a076d05399.js" crossorigin="anonymous"></script>
<script src="https://polyfill.io/v3/polyfill.min.js?version=3.52.1&features=fetch"></script>
  <script src="https://js.stripe.com/v3/"></script>
  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
  <script src="./assets/js/scriptStripe.js"></script>
</body>
</html> 